...
def Search(results, media, lang, manual, movie):
  ...
  for element in AniDBTitlesDB.xpath(u"/animetitles/anime/title[text()[contains(lower-case(.), '%s')]]" % orig_title.lower().replace("'", " ")):
    ...
    title = element.text
    if aid == best_aid and best_score >= 100:
        continue
    if orig_title == title:
        title_cleansed, score = title, 100
    ...
    if score >= 100 and not aid == best_aid:
        n += 1
  ...
  for element in AniDBTitlesDB.xpath(u"/animetitles/anime[title[{}]]".format(" or ".join(["contains(lower-case(text()), '{}')".format(x.lower()) for x in words]) )):
    ...
    if best_score_entry < 25:
        last_chance.append((best_score_entry, best_title_entry, best_type_entry, best_lang_entry, aid))
        continue
  ...
  if best_score < 50:  # Add back lower than 25 if nothing above 50
      ...
      if is_primary_entry:  ### for each main anime AniDBid ###
        ...
        if GetXml(xml, 'picture'):
          ...
          if 'en' in language_posters:
            rank = (rank // 30) * 30 * language_posters.index('en') + rank % 30
        ...
        SPECIAL_TAGS = {"nudity": 300, "sex": 400}

        minimum_weight_preference = int(Prefs['MinimumWeight'] or 200)
        for tag in xml.xpath('tags/tag'):
          tag_weight = int(tag.get('weight', '') or 200)
          tag_name = GetXml(tag, 'name')
          if tag_name and tag.get('weight', '').isdigit() and tag_weight >= minimum_weight_preference:
            data_tag_name = tag_name.lower()
            if data_tag_name not in ignored_tags:
              display_tag_name = string.capwords(tag_name, '-')
              if data_tag_name in SPECIAL_TAGS:
                  if tag_weight >= SPECIAL_TAGS[data_tag_name]:
                    SaveDict([display_tag_name], AniDB_dict, 'genres')
              else:
                SaveDict([display_tag_name], AniDB_dict, 'genres')
            if data_tag_name in RESTRICTED_GENRE:  AniDB_dict['content_rating'] = RESTRICTED_GENRE[data_tag_name]
        ...
        Log.Info("[ ] genres ({}/{} above {} weight): {}".format(len(Dict(AniDB_dict, 'genres')), len(xml.xpath('tags/tag')), minimum_weight_preference, Dict(AniDB_dict, 'genres')))
      ...
      ### Series ###
        ...
        for ep_obj in sorted(xml.xpath('episodes/episode'), key=lambda x: [int(x.xpath('epno')[0].get('type')), int(x.xpath('epno')[0].text if x.xpath('epno')[0].text.isdigit() else x.xpath('epno')[0].text[1:])]):
          ...
          if not anidb_numbering and title == 'Complete Movie':
              title = ""  # For mapping use meaningful titles
          epNum = ep_obj.xpath('epno')[0]
          epNumType = epNum.get('type')
          season = "1" if epNumType == "1" else "0"
          if epNumType == "3" and ep_obj.xpath('title')[0].text.startswith('Ending') and int(epNum.text[1:]) - 1 < ending_offset:
              ending_offset = int(epNum.text[1:]) - 1
          if epNumType == "3" and int(epNum.text[1:]) > ending_offset:
              episode = str(int(epNum.text[1:])+150-ending_offset)  #shifted to 150 for 1st ending.
          elif epNumType == "1":
              episode = epNum.text
          else:
              episode = str(specials[epNum.text[0]][0] + int(epNum.text[1:]))
          ...
          if source.startswith("tvdb") or source.startswith("anidb") and not movie and max(map(int, media.seasons.keys()))>1:
            ...
            if source in ('tvdb3', 'tvdb4') and season not in ['-1', '0']:
              if source == 'tvdb4' or season == '1':
                ...
                try:
                    episode = list(Dict(mappingList, 'absolute_map', default={}).keys())[list(Dict(mappingList, 'absolute_map', default={}).values()).index((season, episode))]
                except:
                    pass
